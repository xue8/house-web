  ...
  export default {
    ...
    methods: {
      ...
      start() {
        ...
        myGeo.getPoint(_this.nearby.address + _this.nearby.location, function (point) {
            if (point) {
              ...
              _this.$axios.get('http://47.93.41.158:8090/map/search' + '?query=' + _this.searchTypeValue +
                  '&location=' + location +
      ...
      house(row) {
        ...
        this.$axios.get('http://47.93.41.158:8090/house/search' + url)
          .then(response => {
            if (response.data.code === 200) {
      ...
      ziroomHouse(row) {
        ...
        this.$axios.get('http://47.93.41.158:8090/house/ziroomSearch' + url)
          .then(response => {
            if (response.data.code === 200) {
              this.houseSearchResult = response.data.result;
              this.houseTotalCount = response.data.totalCount;
