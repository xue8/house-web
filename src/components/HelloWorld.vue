<template>
  <div class="hello">
    <div style="margin-bottom: 20px" id="operation">
      ...
      <br>
      <br>
      <el-tag>暂时仅提供贝壳租房、自如的房源，且为实时爬虫，所以切勿频繁操作，以防服务器 IP 被封。</el-tag>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 开始检索
      start() {
        if (this.nearby.location == null || this.nearby.address == null || this.coordTypeValue == null || this.nearby.radius == null) {
          this.$message.error({
        ...
        myGeo.getPoint(_this.nearby.address + _this.nearby.location, function (point) {
            if (point) {
              ...
              _this.$axios.get('/api/map/search' + '?query=' + _this.searchTypeValue + '&location=' + location +
